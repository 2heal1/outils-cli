...
async function create(projectName) {
  ...
  try {
    ...
    if (mode === CONSTANT.MODE.NORMAL) {
      ...
      if(!config){
        log.error('未找到 outils-config.js 文件，请使用 outils config [option]获取相关配置！');
        return;
      }
      meta = Object.assign(meta,config)
    }
    // 开始下载
    const spinner = ora("🚀  正在下载模板...");
    spinner.start();
    download(
      "github.com:2heal1/outils-cli-template#master",
      projectName,
      { clone: true },
      (err) => {
        if (err) {
    ...
    if(!config){
      log.error('未找到 outils-config.js 文件，请使用 outils config get 获取相关配置！');
    }else{
      log.error(err)
    }
    return;
